/**
 * Modified from [node-ytsr](https://github.com/TimeForANinja/node-ytsr)
 * MIT License
 */
...
const main = async (searchString, options, rt = 3) => {
  if (rt === 0) throw new Error('Unable to find JSON!')
  // Set default values
  const opts = UTIL.checkArgs(searchString, options)

  const ref = BASE_SEARCH_URL + QS.stringify(opts.query)

  const body = await (await fetch(ref, opts.requestOptions)).text()
  ...
  if (parsed.json.alerts && !parsed.json.contents) {
    ...
    if (error)
      throw new Error(
        `API-Error: ${UTIL.parseText(
          error.alertRenderer.text,
          '* no message *'
        )}`
      )
  }
  ...
  resp.activeFilters = Array.from(filters)
    .map((a) => a[1].active)
    .filter((a) => a)
  ...
  if (token && opts.limit === Infinity)
    resp.continuation = [parsed.apiKey, token, parsed.context, opts]
...
const parsePage2 = async (apiKey, token, context, opts) => {
  ...
  if (!nextToken || opts.limit < 1 || opts.pages < 1) {
    return {
      continuation:
        nextToken && opts.limit === Infinity
          ? [apiKey, nextToken, context, opts]
          : null,
      items: parsedItems,
    }
  }
...
export const continueReq = async (args) => {
  // Check params
...
export const getFilters = async (searchString, options) => {
  const opts = UTIL.checkArgs(searchString, options)
  ...
  if (!parsed.json) return getFilters(searchString, options)
