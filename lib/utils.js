...
export const parseFilters = (json) => {
  ...
  const filterWrapper = (wrapper.subMenu || wrapper.submenu)
    .searchSubMenuRenderer.groups
...
// Parsing utility
export const parseText = (txt, def = null) => {
...
export const parseIntegerFromText = (txt) =>
  Number(parseText(txt).replace(/\D+/g, ''))

// Request Utility
export const doPost = async (url, payload, reqOpts = {}) => {
  // Enforce POST-Request
  reqOpts.method = 'POST'
  const req = await fetch(url, reqOpts, {
...
export const checkArgs = (searchString, options = {}) => {
  // Validation
  if (!searchString) {
...
export const parseWrapper = (primaryContents) => {
  let rawItems = []
...
export const parsePage2Wrapper = (continuationItems) => {
  let rawItems = []
  let continuation = null
