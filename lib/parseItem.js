...
export default (...params) => parseItem(...params)
...
const parseVideo = (obj) => {
  ...
  const authorImg = !ctsr
    ? { thumbnail: { thumbnails: [] } }
    : ctsr.channelThumbnailWithLinkRenderer
  const isOfficial = !!(
    obj.ownerBadges && JSON.stringify(obj.ownerBadges).includes('OFFICIAL')
  )
  const isVerified = !!(
    obj.ownerBadges && JSON.stringify(obj.ownerBadges).includes('VERIFIED')
  )
  const lengthFallback = obj.thumbnailOverlays.find(
    (x) => Object.keys(x)[0] === 'thumbnailOverlayTimeStatusRenderer'
  )
  const length =
    obj.lengthText ||
    (lengthFallback && lengthFallback.thumbnailOverlayTimeStatusRenderer.text)
  ...
  return {
    ...
    views: !obj.viewCountText
      ? null
      : UTIL.parseIntegerFromText(obj.viewCountText),
...
const parseChannel = (obj) => {
  ...
  return {
    ...
    videos: obj.videoCountText
      ? UTIL.parseIntegerFromText(obj.videoCountText)
      : null,
  }
}
...
const parseDidYouMeanRenderer = (obj, resp) => {
  ...
  if (resp && Array.isArray(resp.refinements)) {
    resp.refinements.unshift({
      ...
      url: new URL(
        obj.correctedQueryEndpoint.commandMetadata.webCommandMetadata.url,
        BASE_VIDEO_URL
      ).toString(),
      bestThumbnail: null,
      thumbnails: null,
...
const parseHorizontalCardListRenderer = (obj, resp) => {
  ...
  switch (subType) {
    ...
    default:
      throw new Error(
        `subType ${subType} of type horizontalCardListRenderer not known`
      )
  }
}
...
const parseHorizontalRefinements = (obj, resp) => {
  // Add to refinements
  if (resp && Array.isArray(resp.refinements)) {
    resp.refinements.push(
      ...obj.cards.map((c) => {
        const targetUrl =
          c.searchRefinementCardRenderer.searchEndpoint.commandMetadata
            .webCommandMetadata.url
        return {
          q: UTIL.parseText(c.searchRefinementCardRenderer.query, ''),
          url: new URL(targetUrl, BASE_VIDEO_URL).toString(),
          bestThumbnail: prepImg(
            c.searchRefinementCardRenderer.thumbnail.thumbnails
          )[0],
          thumbnails: prepImg(
            c.searchRefinementCardRenderer.thumbnail.thumbnails
          ),
...
const parseHorizontalChannelList = (obj) => {
  ...
  return {
    ...
    channels: obj.cards
      .map((i) => parseHorizontalChannelListItem(i.previewCardRenderer))
      .filter((a) => a),
  }
}
...
const parseHorizontalChannelListItem = (obj) => {
  const thumbnailRenderer =
    obj.header.richListHeaderRenderer.channelThumbnail
      .channelThumbnailWithLinkRenderer
  return {
    type: 'channelPreview',
    name: UTIL.parseText(obj.header.richListHeaderRenderer.title, ''),
    channelID:
      obj.header.richListHeaderRenderer.endpoint.browseEndpoint.browseId,
    url: new URL(
    ...
    videos: obj.contents
      .map((i) => parseVideo(i.gridVideoRenderer))
      .filter((a) => a),
  }
}
...
const parseMovie = (obj) => {
  // Normalize
  obj.bottomMetadataItems = (obj.bottomMetadataItems || []).map((x) =>
    UTIL.parseText(x)
  )
  const actorsString = obj.bottomMetadataItems.find((x) =>
    x.startsWith('Actors')
  )
  const directorsString = obj.bottomMetadataItems.find((x) =>
    x.startsWith('Director')
  )
...
const parseShort = (obj) => {
  ...
  const reelPlayerOverlayRenderer =
  ...
  if (reelPlayerOverlayRenderer.reelPlayerHeaderSupportedRenderers) {
    ...
    core.channel = {
      ...
      url: new URL(
        channelNavEndpoint.commandMetadata.webCommandMetadata.url,
        BASE_VIDEO_URL
      ).toString(),
...
const parseShow = (obj) => {
  const thumbnails =
    obj.thumbnailRenderer.showCustomThumbnailRenderer.thumbnail.thumbnails
  ...
  return {
    ...
    episodes: UTIL.parseIntegerFromText(
      obj.thumbnailOverlays[0].thumbnailOverlayBottomPanelRenderer.text
    ),
    owner,
  }
}
...
const parseShelf = (obj) => {
  ...
  if (Array.isArray(obj.contents)) {
    ...
    rawItems = (
      obj.content.verticalListRenderer ||
      obj.content.horizontalMovieListRenderer
    ).items
  }
...
const _parseOwner = (obj) => {
  const owner =
    (obj.shortBylineText && obj.shortBylineText.runs[0]) ||
